...
<script type="text/javascript">
									...
									.on(
											...
											function(obj) {
												...
												case 'delete':
													...
													break;
												case 'export':
													window.location.href = "/situOA/prod/export?name="
															+ vname
															+ "&code="
															+ vcode;
													break;
												case 'download':
													window.location.href = "/situOA/prod/export?stop=1";
													break;
												}
											});
							upload.render({
	...
	function del(vid) {
		if (!confirm("确认删除该员工？"))
			return;
		ajax("post", "/situOA/prod/deleteModel", {
			id : vid
		}, "text", function(data) {
			if (data == "1")
				layui.use('layer', function() {
	...
	function upd(vid) {
		layui.use('layer', function() {
			...
			layer.open({
				type : 2,
				content : "/situOA/prod/about?id=" + vid,
				area : [ '500px', '550px' ]
			})
		})
	}
